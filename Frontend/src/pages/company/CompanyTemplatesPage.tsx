...
export const CompanyTemplatesPage: React.FC = () => {
  ...
  const [evaluatorFilter, setEvaluatorFilter] = useState<'all' | 'company' | 'college' | 'both' | 'none'>('all');
  ...
  // Quick Settings Form States
  const [quickTitle, setQuickTitle] = useState('');
  const [quickDescription, setQuickDescription] = useState('');
  const [quickDueDate, setQuickDueDate] = useState('');
  const [quickEvaluatorMode, setQuickEvaluatorMode] = useState<'company_only' | 'college_only' | 'both' | 'none'>('company_only');
  ...
  const handleCreateTemplate = async (dto: SaveTemplateDto) => {
    try {
      ...
      toast.success('Company report template created successfully');
    ...
    if (item.requiresCollegeEvaluation && item.requiresCompanyEvaluation) {
      return (
        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-700 border border-indigo-200 dark:bg-indigo-950/60 dark:text-indigo-300 dark:border-indigo-800">
          ...
          <span>Both (Company & College)</span>
        </span>
      );
    }
      ...
      cell: (item) => (
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            className="p-1.5 hover:text-teal-600 hover:bg-teal-50 dark:hover:bg-teal-950"
          >
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <span>{t('nav.reportTemplates', 'Company Report Templates')}</span>
          </h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Configure dynamic evaluation forms, weekly task logs, due dates, evaluator workflows, and publish questionnaires to trainees.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="p-4 bg-gradient-to-br from-teal-50/50 to-white dark:from-slate-900 dark:to-slate-800 border-teal-100 dark:border-slate-800">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            ...
            <span className="text-xs text-teal-600 font-medium">visible to trainees</span>
          </div>
        </Card>
        ...
        <Card className="p-4 bg-gradient-to-br from-indigo-50/50 to-white dark:from-slate-900 dark:to-slate-800 border-indigo-100 dark:border-slate-800">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Trainee Responses</span>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 p-3 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xs">
        <div className="flex-1 max-w-md">
          <Input
            placeholder="Search company templates by title or description..."
            value={searchTerm}
      ...
      <Table
        ...
        isLoading={loading}
        emptyMessage="No company report templates match your search or filter criteria."
      />
      ...
      <Modal
        ...
        title="Create New Company Report Template"
        maxWidth="2xl"
      >
      ...
      <Modal
        ...
        title="Edit Company Report Template & Settings"
        maxWidth="2xl"
      >
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  className="w-full text-xs rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 p-2.5 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>
              ...
              <div className="pt-2">
                <label className="flex items-center justify-between p-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 cursor-pointer">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[11px] text-slate-400">
                      {quickIsAvailable ? 'Visible and ready for submissions' : 'Hidden from trainees (Draft)'}
                    </p>
                  </div>
                  <input
                    type="checkbox"
      ...
      <Modal
        ...
        title="Form Preview (Trainee View)"
        maxWidth="lg"
      >
        {activeTemplateDetails && (
          <div className="space-y-5">
            ...
            <div className="p-4 rounded-xl border border-emerald-100 dark:border-emerald-950/60 bg-emerald-50/40 dark:bg-emerald-950/30 space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-3 pt-2 text-[11px] text-slate-500 dark:text-slate-400 border-t border-emerald-100/60 dark:border-emerald-950">
                <span>
                ...
                <span>•</span>
                <span>
                  Evaluators: <strong className="text-slate-700 dark:text-slate-300">{activeTemplateDetails.requiresCollegeEvaluation && activeTemplateDetails.requiresCompanyEvaluation ? 'Both' : activeTemplateDetails.requiresCompanyEvaluation ? 'Company Only' : activeTemplateDetails.requiresCollegeEvaluation ? 'College Only' : 'None'}</strong>
                </span>
              </div>
            </div>
              ...
              {activeTemplateDetails.questions.map((q, idx) => (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className="text-[10px] font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950 px-2 py-0.5 rounded">
                      {q.questionType}
                    </span>
                  </div>
